...
# Prefer reading the UUID from the already-mounted filesystem (findmnt asks
# the kernel directly for the UUID of the fs backing $MNT_ROOT) rather than
# blkid -p on the raw block device while it's still mounted underneath us —
# the latter has been observed to occasionally return a stale/mismatched
# UUID (root cause of a "VFS: Cannot open root device" boot panic where
# grub.cfg ends up with a different UUID than the actual filesystem).
ROOT_UUID=""
if command -v findmnt >/dev/null 2>&1; then
    ROOT_UUID="$(findmnt -no UUID "$MNT_ROOT" 2>/dev/null || true)"
fi

# Fallback: blkid -p (direct probe mode) on the block device.
if [ -z "$ROOT_UUID" ]; then
    ROOT_UUID="$(blkid -p -s UUID -o value "$ROOT_DEV")"
fi

# Last resort: blkid without -p.
if [ -z "$ROOT_UUID" ]; then
...
# Sanity check: cross-verify against blkid -p on the raw device. If the two
# disagree, something is genuinely inconsistent (most likely a stale loop
# device reused from a previous, not-fully-cleaned-up run) — better to fail
# loudly here than to bake a mismatched UUID into grub.cfg and get a much
# more confusing kernel panic at boot time instead.
...
if [ -n "$BLKID_UUID" ] && [ "$BLKID_UUID" != "$ROOT_UUID" ]; then
    ...
    echo "[!] This usually means a stale loop device from a previous run."
    echo "[!] Run 'sudo losetup -a', detach any leftover loop devices bound"
    echo "    to old/deleted images, then re-run this script."
    exit 1
fi
...
# Unique identifier for THIS build run. Embedded below as a harmless extra
# kernel cmdline token, so every serial log line 1 ("Kernel command line: ...")
# tells you unambiguously which build-disk.sh run produced the image you're
# looking at — no more guessing whether you're booting a stale image.
